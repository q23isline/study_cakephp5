<template>
  <div>
    ...
    <ul class="nav justify-content-end">
      <li class="nav-item">
        <RouterLink class="nav-link link-primary" aria-current="page" :to="`/users/${props.userId}`"
          >詳細</RouterLink
        >
      </li>
      <li class="nav-item">
...
const onSubmit = async () => {
  ...
  try {
    await AppApi.patch<UserSaveApiResponse>(`/api/v1/sample-users/${props.userId}`, params)

    // TODO: 更新しました。のトーストメッセージを出したい。
  } catch (err) {
...
const load = async () => {
  try {
    ...
    birthDay.value = DateUtil.formatDateHyphen(response.data.data.attributes.birth_day)
    height.value = response.data.data.attributes.height
    gender.value = response.data.data.attributes.gender
  } catch (err) {
    if (err instanceof AxiosError) {
      if (err.response?.status === 404) {
